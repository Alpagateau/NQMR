...
int main( void ) {
  ...
  bool is_playing = true;
    ...
    for(int i = 0; i < TEST_BUFFER_SIZE; i++)
    {
      ...
      player_anim.Play_then(&ollie, &idle);
    if(NQMT::isButtonJustPressed( KEY_B ))
      player_anim.Play_then(&kick, &idle);
    if(NQMT::isButtonJustPressed( KEY_X ))
      player_anim.Play_then(&shove, &idle);
